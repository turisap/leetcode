...
/*
     ...
     6) create cpu profile  with count and compare
       go test -count=10  -bench=BenchmarkSDUUp -run=XXX -benchmem  > stat1 &&
       go test -count=10  -bench=BenchmarkSDUDown -run=XXX -benchmem > stat2 &&
       benchstat stat1 stat2
*/

func BenchmarkSDUDown(b *testing.B) {
	for i := 0; i < b.N; i++ {
		sharedDown(100)
	}
}

func BenchmarkSDUUp(b *testing.B) {
	for i := 0; i < b.N; i++ {
		sharedUp(100)
	}
}
