...
/*
     ...
     6) create cpu profile  with count and compare
       go test -count=10  -bench=BenchmarkPLLRecursive -run=XXX -benchmem  > stat1 &&
       go test -count=10  -bench=BenchmarkPLLIterative -run=XXX -benchmem > stat2 &&
       benchstat stat1 stat2
*/

func BenchmarkPLLIterative(b *testing.B) {
	for i := 0; i < b.N; i++ {
		pllIterative(testCases[0].head)
	}
}
